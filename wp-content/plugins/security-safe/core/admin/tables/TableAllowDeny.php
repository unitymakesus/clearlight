	...
	final class TableAllowDeny extends Table {
		...
		function get_columns() {
			...
			return [
				...
				'expire_status' => __( 'Status', SECSAFE_SLUG ),
		...
		function column_expire_status( $item ) {
			...
			$expire_status = ( $item->date_expire == '0000-00-00 00:00:00' || date( 'Y-m-d H:i:s' ) < $item->date_expire ) ? __( 'active', SECSAFE_SLUG ) : __( 'expired', SECSAFE_SLUG );
		...
		function add_ip() {
			...
			if (
				...
				if ( ! $nonce || ! wp_verify_nonce( $nonce, SECSAFE_SLUG . '-add-ip' ) ) {
					...
					$this->messages[] = [
						__( 'Error: IP address not added. Your session expired. Please try again.', SECSAFE_SLUG ),
						3,
					];
				...
				if ( $ip && $expire !== false ) {
					...
					if ( $result ) {
						...
						$SecuritySafe->messages[] = [
							sprintf( __( 'Notice: %1$s -  IP address is already in the database.', SECSAFE_SLUG ), $ip ),
							2,
							0,
						];
						...
						if ( $result ) {
							...
							$SecuritySafe->messages[] = [
								sprintf( __( '%1$s - IP address added with %2$s rule.', SECSAFE_SLUG ), $ip, $args['status'] ),
								0,
								0,
							];
							...
							$SecuritySafe->messages[] = [
								sprintf( __( 'Error: %1$s - IP address could not be added. Unknown reason.', SECSAFE_SLUG ), $ip ),
								3,
								0,
							];
					...
					if ( ! $ip ) {
						...
						$SecuritySafe->messages[] = [
							sprintf( __( 'Error: %s - IP address not valid.', SECSAFE_SLUG ), esc_html( $_REQUEST['ip'] ) ),
							3,
							0,
						];
						...
						$SecuritySafe->messages[] = [
							sprintf( __( 'Error: %s - Timespan not valid.', SECSAFE_SLUG ), esc_html( $_REQUEST['ip_expire'] ) ),
							3,
							0,
						];
		...
		public function check_whitelist() {
			...
			if ( $ip != '::1' ) {
				...
				if ( ! $whitelisted ) {
					...
					$SecuritySafe->messages[] = [
						sprintf( __( '%s We recommend adding your IP to the allow list using the form below.', SECSAFE_SLUG ), $ip ),
						2,
						0,
					];
